...
El catálogo vive en `backend/src/constants/achievements.ts` y se sincroniza mediante `upsert` por `code`, por lo que inicializarlo varias veces es seguro. Los desbloqueos se guardan en `UserAchievement`; el progreso restante se calcula desde los registros existentes y no duplica estadísticas derivables. El backend comprueba la conexión con Prisma e inicializa automáticamente el catálogo antes de escuchar conexiones, incluido el arranque de producción con `node dist/server.js`.
...
npm run prisma:generate
npm run prisma:migrate
# Opcional: reinicialización manual del catálogo en desarrollo
...
No es necesario ejecutar el seed manualmente en producción: el arranque del backend aplica los mismos `upsert`. Si la conexión o la inicialización falla, la API no abre el puerto y termina con código de error.
